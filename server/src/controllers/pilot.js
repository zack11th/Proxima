...
let orbit = {
    ...
    planets: [
        ...
        },
        {
            ...
            F: -20, // наклон
            P: 0,
            w: 60, // угол относительно центра (изменяется по интервалу)
            v: 0, // скорость (считает автоматом)
            W_speed: 0, // угловая скорость  (считает автоматом)
            h: 0, // высота  (считает автоматом)
            shift: 0 // смещение (считает автоматом)
    ...
    ship: {
        name: 'Aurora',
        a: 6500000, // большая полуось
        b: 5000000, // малая полуось
        F: -20, // наклон
        P: 0,
        w: 60, // угол относительно центра (изменяется по интервалу)
...
function calcSpeed(items) {
    items.forEach((item, i, arr) => {
        ...
        item.w_speed = item.v/item.h*180/Math.PI;
        item.w -= item.w_speed
    });
}

function calcSpeedShip(ship) {
    ship.w_speed = ship.v/ship.a*180/Math.PI;
    ship.w -= ship.w_speed;
}
